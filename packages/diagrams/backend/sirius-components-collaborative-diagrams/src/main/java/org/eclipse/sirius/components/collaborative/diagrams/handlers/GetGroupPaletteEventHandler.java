...
@Service
public class GetGroupPaletteEventHandler implements IDiagramEventHandler {
    ...
    public GetGroupPaletteEventHandler(IRepresentationDescriptionSearchService representationDescriptionSearchService, IDiagramQueryService diagramQueryService,
    ...
    public boolean canHandle(IDiagramInput diagramInput) {
        return diagramInput instanceof GetGroupPaletteInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IDiagramContext diagramContext, IDiagramInput diagramInput) {
        ...
        String message = this.messageService.invalidInput(diagramInput.getClass().getSimpleName(), GetGroupPaletteInput.class.getSimpleName());
        ...
        if (diagramInput instanceof GetGroupPaletteInput toolSectionsInput) {
            ...
            if (optionalDiagramDescription.isPresent()) {
                ...
                var targetElements = toolSectionsInput.diagramElementIds().stream()
                        .map(id -> this.findTargetElement(diagram, id, editingContext).orElse(null))
                        .toList();
                var diagramElements = toolSectionsInput.diagramElementIds().stream()
                        .map(id -> this.findDiagramElement(diagram, id).orElse(null))
                        .toList();
                // [FRO] Is there a better way to do that?
                var diagramElementDescriptions = toolSectionsInput.diagramElementIds().stream()
                        // TODO don't call again findDiagramElement
                        .map(id -> this.findDiagramElementDescription(diagram, id, diagramDescription, this.findDiagramElement(diagram, id)))
                        .toList();
                if (optionalToolSectionsProvider.isPresent() && !targetElements.isEmpty() && !diagramElements.isEmpty()) {
                    IPaletteProvider toolSectionsProvider = optionalToolSectionsProvider.get();
                    palette = toolSectionsProvider.handle(targetElements, diagramElements, diagramElementDescriptions,
                            diagramDescription, editingContext);
                }
            }
        }
        if (palette != null) {
            payload = new GetGroupPaletteSuccessPayload(diagramInput.id(), palette);
        }
